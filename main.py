...
def generate_title(model, tokenizer, history: list) -> str:
...
def stream_response(stream, show_thinking: bool = True) -> str:
    """
...
def save_chat(history: list, path: Path, title: str) -> None:
...
def prompt_save(model, tokenizer, history: list, resume: str | None) -> None:
    ...
    if resume:
        ...
        if overwrite == "y":
            ...
            save_chat(history, Path(resume), title)
    ...
    if mode == "e":
        ...
        suggested = generate_title(model, tokenizer, history)
    ...
    save_chat(history, path, slug)
    print(f"Saved to {path}")


def chat(resume: str | None = None, show_thinking: bool = True, max_turns: int | None = None):
    print("Loading model...")
    ...
    while True:
        ...
        print("Nemotron:")
        stream = stream_generate(model, tokenizer, prompt=formatted, max_tokens=4096)
        full_response = stream_response(stream, show_thinking=show_thinking)
        print()

        history.append({"role": "assistant", "content": full_response})

    if history:
        prompt_save(model, tokenizer, history, resume)
...
if __name__ == "__main__":
    ...
    chat(resume=args.resume, show_thinking=not args.no_thinking, max_turns=args.max_turns)
